...
model = dict(
    ...
    tracker=dict(
        ...
        init_score_thr=0.1,
        obj_score_thr=0.01,
        match_score_thr=0.4,       # lower = easier to re-match (was 0.5)
        memo_tracklet_frames=322,   # remember objects for 100 frames (was 10)
        memo_momentum=0.8,
        with_cats=False,
        max_distance=-1,            # no distance limit (drone view shifts a lot)
        fps=1,                      # ~1 fps for 2s-interval drone images
        )
)
